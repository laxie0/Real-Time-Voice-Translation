...
def update_input_lang_code(event):
    ...
    if selected_language_name in language_codes:
        input_lang.set(language_codes[selected_language_name])
...
def update_output_lang_code(event):
    selected_language_name = event.widget.get()
    if selected_language_name in language_codes:
        output_lang.set(language_codes[selected_language_name])
    ...
    if keep_running:
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                win.after(0, lambda: input_text.insert(tk.END, f"{speech_text_transliteration}\n"))

                if speech_text.lower() in {'exit', 'stop'}:
                    keep_running = False
                    return
                
                source_lang = input_lang.get() if input_lang.get() != "auto" else "en"
                translated_text = GoogleTranslator(source=source_lang, target=output_lang.get()).translate(text=speech_text_transliteration)
                ...
                win.after(0, lambda: output_text.insert(tk.END, translated_text + "\n"))
                
            except sr.UnknownValueError:
                win.after(0, lambda: output_text.insert(tk.END, "Could not understand!\n"))
            except sr.RequestError:
                win.after(0, lambda: output_text.insert(tk.END, "Could not request from Google!\n"))
